function [area, points, new_area, empty_area] = heuristic_boundary_double(area, parking_size, algorithm, index, rot_index)
% Purpose: Place two rows of parking slots along all the sides of an
% arbitrary polygon
...
% First we place the first row
point = parking_size(1:2)/2;
%xmax = find_xmax(point(2)-parking_size(2)/2, point(2)+parking_size(2)/2, area);
%xmin = find_xmin(point(2)-parking_size(2)/2, point(2)+parking_size(2)/2, area);
x_interval = find_x_interval(point(2)-parking_size(2)/2, point(2)+parking_size(2)/2, area);
...
while xmax - point(1) >= parking_size(1)*0.5
    points = [points; [point, 0, 1]];
    point = point + [parking_size(1), 0];
end

% If we can't place any points along the first side, then we can't place
% any rows, so we return
if size(points, 1) == 0
    points = [];
    new_area = [];
    empty_area = area*rot_mat^-1;
...
last_lower_point = points(end, 1:2); % save the last point placed

% Now we place the second row
point = [parking_size(1)/2, 3*parking_size(2)/2];

%xmax = find_xmax(0, point(2)+parking_size(2)/2, area);
%xmin = find_xmin(0, point(2)+parking_size(2)/2, area);
x_interval = find_x_interval(0, point(2)+parking_size(2)/2, area);
if length(x_interval) > 2
    disp('SOMETHING IS WRONG')
end
xmin = x_interval(1);
...
last_upper_point = points(end, 1:2); % save the last point placed

% save or initiate variables
prev_direction = [1, 0];
prev_angle = zeros(n, 1);
prev_point = [0, 0] + [0, parking_size(2)*2+parking_size(3)];
no_points = 0;
index_last_point = 1;
been_over_min_y_lower = 0;
been_over_min_y_upper = 0;
prev_direction_last_point = [1, 0];
prev_normal_last_point = [0, 1];
been_below_0_x_lower = 0;
been_below_0_x_upper = 0;
...
for i=2:n
    ...
    % if the length of a side is too small, we return
    if norm(side) < 1e-5 && i == n
        new_area = [];
        empty_area = area*rot_mat^-1;
        empty_area = empty_area + displacement;
        return;
    ...
    %% CONSIDER CONFLICTS WITH PREVIOUS LOWER ROW
    
    % We subtract 1e-3, such that angles very close to 90 degrees, we end
    % in the correct case
    if alpha >= pi/2 - 1e-3
        % Trigonometric observations
        red_side = sin(alpha-pi/2)*parking_size(2);
        long_side_lower = (red_side + parking_size(2))/cos(alpha-pi/2);
    else
        
        % The case where the side of the next slot, hits the corner of the
        % actual previous parking slot
        corner = last_lower_point + parking_size(1)/2*prev_direction_last_point + parking_size(2)/2*prev_normal_last_point;
        ...
        t = [direction_vector', -prev_normal_last_point']\(last_lower_point+parking_size(1)/2*prev_direction_last_point-area(i, :))';
        C = t(1)*direction_vector + area(i, :);
        
        long_side_2 = red_side/cos(alpha);
        long_side_2 = long_side_2 - norm(C-B, 2);
        
        long_side_lower = min(long_side_1, long_side_2);
        long_side_lower = max(long_side_lower, 0);
        
    end
    
    
    %% CONSIDER CONFLICTS WITH PREVIOUS UPPER ROW
    
    if alpha >= pi/2 - 1e-3
        % Trigonometric observations
        red_side = sin(alpha-pi/2)*parking_size(2);
        long_side_upper = (red_side + 2*parking_size(2) + parking_size(3))/cos(alpha-pi/2);
    else
        % The case where the side of the next slot, hits the corner of the
        % free space in front of the previous parking slot
        corner = last_upper_point + parking_size(1)/2*prev_direction_last_point + (parking_size(2)/2 + parking_size(3))*prev_normal_last_point;
        t = [direction_vector', -normal_vector']\(corner - area(i, :))';
        A = t(1)*direction_vector + area(i, :);
        ...
        % The case where the side of the next slot, hits the corner of the
        % actual previous parking slot
        corner = last_upper_point + parking_size(1)/2*prev_direction_last_point + parking_size(2)/2*prev_normal_last_point;
        t = [direction_vector', -normal_vector']\(corner - area(i, :))';
        A = t(1)*direction_vector + area(i, :);
        ...
        t = [direction_vector', -prev_normal_last_point']\(last_upper_point+parking_size(1)/2*prev_direction_last_point-area(i, :))';
        C = t(1)*direction_vector + area(i, :);
        
        long_side_2 = red_side/cos(alpha);
        long_side_2 = long_side_2 - norm(C-B, 2);
        
        long_side_upper = min(long_side_1, long_side_2);
        long_side_upper = max(long_side_upper, max(min_long_side, 0));
        
    end
    
    long_side = max(long_side_lower, long_side_upper);
    
    point = B + parking_size(2)/2*normal_vector;
    point = point + (long_side + parking_size(1)/2)*direction_vector;
    
    real_point = point + parking_size(2)*normal_vector;
    
    % Solve for a corner of the new polygon
    if index
        t = [direction_vector', -prev_direction']\(prev_point-real_point-parking_size(2)/2*normal_vector)';
        new_area = [new_area; t(1)*direction_vector + real_point + parking_size(2)/2*normal_vector];
    else
        t = [direction_vector', -prev_direction']\(prev_point-real_point-(parking_size(2)/2+parking_size(3))*normal_vector)';
        new_area = [new_area; t(1)*direction_vector + real_point + (parking_size(2)/2 + parking_size(3))*normal_vector];
    end
    ...
    % next side/sides
    point_on_side = (area(i, :) + area(k, :))/2 + normal_vector*parking_size(2);
    top_intersection_point  = find_next_intersection(area, area(k, :) - area(i, :), i, point_on_side);
    stopping_point_1 = top_intersection_point - parking_size(1)/2*direction_vector - parking_size(2)/2*normal_vector;
    ...
    if been_over_min_y_lower
        min_y = min(rotated_slot(:, 2));
    else
        min_y = inf;
    end
    if x_max_constraint_holds || been_below_0_x_lower
        max_x = max(rotated_slot(1, :));
    else
        max_x = inf;
    end
    
    % Place the points in the lower row along this side
    while norm(stopping_point - point) > parking_size(1)/2 && (stopping_point - point)*direction_vector' > 0 && (parking_size(2) + parking_size(3) <= point(2) + 3*min_y || 0 > point(1)+max_x)
        points = [points; [point, angle/pi*180, 1]];
        point = point + parking_size(1)*direction_vector;
        index_last_point = i;
    end
    last_lower_point_now = points(end, 1:2 );
    
    % Check if we need to consider the minimum x and y value when placing
    % slots along the next side
    min_y = min(rotated_slot(:, 2));
    if ~been_over_min_y_lower && parking_size(2) + parking_size(3) < point(2) + 3*min_y
        been_over_min_y_lower = 1;
    end
    max_x = max(rotated_slot(1, :));
    if ~been_below_0_x_lower && 0 > point(1) + max_x
        been_below_0_x_lower = 1;
    end
    ...
    if alpha < pi/2 - 1e-4
        ...
        % The case where the corner of the next slot, hits the side of the 
        % last previous parking slot
        red_side = sin(alpha)*2*parking_size(2);
    ...
    if alpha >= pi/2 - 1e-4
        % Trigonometric observations
        red_side = sin(alpha-pi/2)*2*parking_size(2);
        long_side_upper = (red_side + 2*parking_size(2) + parking_size(3))/cos(alpha-pi/2);
    else
    ...
    long_side = max(long_side_lower, long_side_upper);
    point = B + 3*parking_size(2)/2*normal_vector;
    point = point + (long_side + parking_size(1)/2)*direction_vector;

    % Find the stopping point, by considering the two places it can hit the
    % next side/sides
    point_on_side = (area(i, :) + area(k, :))/2 + normal_vector*2*parking_size(2);
    top_intersection_point  = find_next_intersection(area, area(k, :) - area(i, :), i, point_on_side);
    stopping_point_1 = top_intersection_point - parking_size(1)/2*direction_vector - (parking_size(2)/2 + parking_size(3))*normal_vector;
    stopping_point_2 = area(k, :) + 3*parking_size(2)/2*normal_vector - parking_size(1)/2*direction_vector;
    if (stopping_point_2 - stopping_point_1)*direction_vector'>0
        stopping_point = stopping_point_1;
    else
        stopping_point = stopping_point_2;
    ...
    % If needed, calculate the minimum y and x value, such that the parking
    % slots don't intersect with the first row of parking slots
    if been_over_min_y_upper
        min_y = min(rotated_slot(:, 2));
    else
        min_y = inf;
    end
    if x_max_constraint_holds || been_below_0_x_upper
        max_x = max(rotated_slot(1, :));
    else
        max_x = inf;
    end
    ...
    last_lower_point = last_lower_point_now;
    last_upper_point = points(end, 1:2);
    
    % Check if we need to consider the minimum x and y value when placing
    % slots along the next side
    min_y = min(rotated_slot(:, 2));
    if ~been_over_min_y_upper && parking_size(2) + parking_size(3) < point(2) + 3*min_y
        been_over_min_y_upper = 1;
    end
        max_x = max(rotated_slot(1, :));
    if ~been_below_0_x_upper && 0 > point(1) + max_x
        been_below_0_x_upper = 1;
    end
    ...
    if index
        prev_point = real_point + parking_size(2)/2*normal_vector;
    else
        prev_point = real_point + (parking_size(2)/2+parking_size(3))*normal_vector;
    end
end

real_point = last_upper_point + (parking_size(2)/2 + parking_size(3))*normal_vector;

% Calculate the last point in the new area
if index
    t = [direction_vector', -[1, 0]']\(2*parking_size(2)*[0,1]-real_point-parking_size(2)/2*normal_vector)';
    new_area = [t(1)*direction_vector + real_point + parking_size(2)/2*normal_vector; new_area];
else
    t = [direction_vector', -[1, 0]']\((2*parking_size(2)+parking_size(3))*[0,1]-real_point)'; % -(parking_size(2)/2+parking_size(3))*normal_vector
    new_area = [t(1)*direction_vector + real_point; new_area];
end
...
if max(new_area(:, 1))-min(new_area(:, 1)) >= max(area(:, 1))-min(area(:, 1)) || max(new_area(:, 2))-min(new_area(:, 2)) >= max(area(:, 2))-min(area(:, 2))
    new_area = [];
    empty_area = area*rot_mat^-1;
    empty_area = empty_area + displacement;
    return;
...
if algorithm == 0
    [new_area, new_points] = heuristic_polygon_straight(new_area, parking_size, 0, 0);
    empty_area = [];
elseif algorithm == 1
    [new_area, new_points, ~, empty_area] = heuristic_boundary_single(new_area, parking_size, 1, 0, 0);
elseif algorithm == 2
    [new_area, new_points, ~, empty_area] = heuristic_boundary_double(new_area, parking_size, 2, 0, 0);
else
    new_points = [];
    empty_area = [];
end
